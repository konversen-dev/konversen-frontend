...
export default function ProfileForm({ user, isEditing, setIsEditing }) {
  ...
  // Saat user klik Save → buka modal
  const handleSaveClick = () => {
    setIsModalOpen(true);
  };

  // Jika user klik Confirm di modal
  const confirmSave = () => {
    setIsModalOpen(false);
    setIsEditing(false);
  };

  // Jika user klik Cancel di modal
  const cancelSave = () => {
    setIsModalOpen(false);
  };
  ...
  return (
    <div className="bg-white shadow-md border rounded-xl p-10 w-full max-w-xl relative">
      ...
      {/* FULL NAME */}
      <FormInput label="Full Name" name="name" value={formData.name} disabled={!isEditing} onChange={handleChange} />

      {/* EMAIL */}
      <FormInput label="Email" name="email" value={formData.email} disabled={!isEditing} onChange={handleChange} />

      {/* PHONE NUMBER */}
      <FormInput label="Phone Number" name="phone" value={formData.phone} disabled={!isEditing} onChange={handlePhoneChange} />

      {/* ADDRESS */}
      <FormInput label="Address" name="address" value={formData.address} disabled={!isEditing} onChange={handleChange} />
